...
import name.kazennikov.common.trove.TIntDeque;
...
public class CompactTreeMap<K,V> implements NavigableMap<K,V>, Cloneable, java.io.Serializable {
     ...
     */
    public CompactTreeMap(Comparator<? super K> comparator) {
     ...
     */
    public CompactTreeMap(Map<? extends K, ? extends V> m) {
        comparator = null;
        putAll(m);
    }
     ...
     * @throws NullPointerException if the specified map is null
     */
    public CompactTreeMap(SortedMap<K, ? extends V> m) {
    ...
    final int getEntryUsingComparator(Object key) {
        ...
        if (cpr != null) {
            int p = root;
            while (p != NIL) {
                int cmp = cpr.compare(k, (K) _key[p]);
                if (cmp < 0)
                    p = _left[p];
                else if (cmp > 0)
                    p = _right[p];
        ...
        while (p != NIL) {
            ...
            if (cmp < 0) {
                ...
                if (_right[p] != NIL) {
                    ...
                    int parent = _parent[p];
                    int ch = p;
                    while (parent != NIL && ch == _right[parent]) {
                        ch = parent;
        ...
        while (p != NIL) {
            ...
            if (cmp > 0) {
                ...
                if (_left[p] != NIL) {
                    ...
                    int parent = _parent[p];
                    int ch = p;
                    while (parent != NIL && ch == _left[parent]) {
                        ch = parent;
                        parent = _parent[parent];
                    }
                    return parent;
    ...
    public void realloc() {
    	if(_parent == null) {
    		_parent = new int[initSize];
    		_left = new int[initSize];
    		_right = new int[initSize];
    		_color = new boolean[initSize];
    		
    		_key = new Object[initSize];
    		_value = new Object[initSize];
    	} else {
    		int[] _parent = new int[this._parent.length << 1];
    		int[] _left = new int[this._left.length << 1];
    		int[] _right = new int[this._right.length << 1];
    		boolean[] _color = new boolean[this._color.length << 1];
    		Object[] _key = new Object[this._key.length << 1];
    		Object[] _value = new Object[this._value.length << 1];
    		
    		System.arraycopy(this._parent, 0, _parent, 0, this._parent.length);
    		System.arraycopy(this._left, 0, _left, 0, this._left.length);
    		System.arraycopy(this._right, 0, _right, 0, this._right.length);
     ...
     */
    public Object clone() {
        CompactTreeMap<K,V> clone = null;
        try {
            clone = (CompactTreeMap<K,V>) super.clone();
    ...
    class Values extends AbstractCollection<V> {
        ...
        public int size() {
            return CompactTreeMap.this.size();
        }

        public boolean contains(Object o) {
            return CompactTreeMap.this.containsValue(o);
        }

        public boolean remove(Object o) {
            for (int e = getFirstEntry(); e != NIL; e = successor(e)) {
                if (valEquals((V) _value[e], o)) {
                    deleteEntry(e);
                    return true;
                }
            }
    ...
    class EntrySet extends AbstractSet<Map.Entry<K,V>> {
        ...
        public boolean contains(Object o) {
            ...
            return p != NIL && valEquals((V) _value[p], value);
        }
        ...
        public boolean remove(Object o) {
            ...
            V value = entry.getValue();
            int p = getEntry(entry.getKey());
            if (p != NIL && valEquals((V) _value[p], value)) {
                deleteEntry(p);
                return true;
            }
            return false;
    ...
    static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
        ...
        public Iterator<E> iterator() {
            if (m instanceof CompactTreeMap)
                return ((CompactTreeMap<E,Object>)m).keyIterator();
            else
        ...
        public Iterator<E> descendingIterator() {
            if (m instanceof CompactTreeMap)
                return ((CompactTreeMap<E,Object>)m).descendingKeyIterator();
            else
    ...
    abstract class PrivateEntryIterator<T> implements Iterator<T> {
        ...
        public void remove() {
            ...
            if (_left[lastReturned] != NIL && _right[lastReturned] != NIL)
                next = lastReturned;
    ...
    Map.Entry<K,V> exportEntry(int e) {
        return (e == NIL) ? null :
            new AbstractMap.SimpleImmutableEntry<>((K)_key[e], (V) _value[e]);
    }
    ...
    abstract static class NavigableSubMap<K,V> extends AbstractMap<K,V>
        ...
        NavigableSubMap(CompactTreeMap<K,V> m,
            ...
            int e =
                (toEnd ?  m.getLastEntry() :
                 (hiInclusive ?  m.getFloorEntry(hi) :
                                 m.getLowerEntry(hi)));
            return (e == NIL || tooLow((K) m._key[e])) ? NIL : e;
        }
        ...
        final int absCeiling(K key) {
            ...
            return (e == NIL || tooHigh(m._key[e])) ? NIL : e;
        }
        ...
        final int absFloor(K key) {
            ...
            return (e == NIL || tooLow(m._key[e])) ? NIL : e;
        }
        ...
        public final NavigableSet<K> navigableKeySet() {
            ...
            return (nksv != null) ? nksv :
                (navigableKeySetView = new CompactTreeMap.KeySet(this));
        }
        ...
        abstract class EntrySetView extends AbstractSet<Map.Entry<K,V>> {
            ...
            public boolean contains(Object o) {
                ...
                return node != NIL &&
                    valEquals((V) m._value[node], entry.getValue());
            }
            ...
            public boolean remove(Object o) {
                ...
                if (!inRange(key))
                    return false;
                int node = m.getEntry(key);
                if (node!=NIL && valEquals((V) m._value[node],
                                            entry.getValue())) {
                    m.deleteEntry(node);
                    return true;
                }
        ...
        abstract class SubMapIterator<T> implements Iterator<T> {
            ...
            SubMapIterator(int first,
                ...
                lastReturned = NIL;
                next = first;
                fenceKey = fence == NIL? UNBOUNDED : m._key[fence];
            }

            public final boolean hasNext() {
                return next != NIL && m._key[next] != fenceKey;
            }

            final Map.Entry<K,V> nextEntry() {
                int e = next;
                if (e == NIL || m._key[e] == fenceKey)
                    throw new NoSuchElementException();
            ...
            final void removeAscending() {
                ...
                if (m.modCount != expectedModCount)
                    throw new ConcurrentModificationException();
                // deleted entries are replaced by their successors
                if (m._left[lastReturned] != NIL && m._right[lastReturned] != NIL)
                    next = lastReturned;
    ...
    static final class AscendingSubMap<K,V> extends NavigableSubMap<K,V> {
        ...
        AscendingSubMap(CompactTreeMap<K,V> m,
                        boolean fromStart, K lo, boolean loInclusive,
                        boolean toEnd,     K hi, boolean hiInclusive) {
    ...
    static final class DescendingSubMap<K,V>  extends NavigableSubMap<K,V> {
        ...
        DescendingSubMap(CompactTreeMap<K,V> m,
                        boolean fromStart, K lo, boolean loInclusive,
                        boolean toEnd,     K hi, boolean hiInclusive) {
            super(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
        }
    ...
    private class SubMap extends AbstractMap<K,V>
        ...
        private Object readResolve() {
            return new AscendingSubMap(CompactTreeMap.this,
    ...
    private boolean colorOf(int p) {
        return (p == NIL? BLACK : _color[p]);
    }

    private int parentOf(int p) {
        return (p == NIL? NIL : _parent[p]);
    }
    ...
    private int leftOf(int p) {
        return (p == NIL) ? NIL : _left[p];
    }

    private int rightOf(int p) {
        return (p == NIL) ? NIL: _right[p];
    }
    ...
    private void rotateLeft(int p) {
        if (p != NIL) {
            ...
            _right[p] = _left[r];
            if (_left[r] != NIL)
                _parent[_left[r]] = p; // r.left.parent = p;
            _parent[r] = _parent[p];
            if (_parent[p] == NIL)
                root = r;
            else if (_left[_parent[p]] == p)
                _left[_parent[p]] = r;
            else
                _right[_parent[p]] = r;
            _left[r] = p;
    ...
    private void rotateRight(int p) {
        if (p != NIL) {
            int l = _left[p];
            _left[p] = _right[l];
            if (_right[l] != NIL) _parent[_right[l]] = p; // l.right.parent 
            _parent[l] = _parent[p];
            if (_parent[p] == NIL)
                root = l;
            else if (_right[_parent[p]] == p)
                _right[_parent[p]] = l;
            else _left[_parent[p]] = l;
            _right[l] = p;
            _parent[p] = l;
        }
    }
    ...
    private void fixAfterInsertion(int x) {
        ...
        while (x != NIL && x != root && _color[_parent[x]] == RED) {
    ...
    private void deleteEntry(int p) {
        ...
        int replacement = (_left[p] != NIL? _left[p] : _right[p]);

        if (replacement != NIL) {
            // Link replacement to parent
            _parent[replacement] = _parent[p];
            if (_parent[p] == NIL)
                root = replacement;
            else if (p == _left[_parent[p]])
                _left[_parent[p]]  = replacement;
            else
                _right[_parent[p]] = replacement;

            // Null out links so they are OK to use by fixAfterDeletion.
            _left[p] = _right[p] = _parent[p] = NIL;
            free.push(p); // FIXME: check

            // Fix replacement
            if (_color[p] == BLACK)
                fixAfterDeletion(replacement);
        } else if (_parent[p] == NIL) { // return if we are the only node.
            root = NIL;
            ...
            if (_parent[p] != NIL) {
                if (p == _left[_parent[p]])
                    _left[_parent[p]] = NIL;
                else if (p == _right[_parent[p]])
                    _right[_parent[p]] = NIL;
                _parent[p] = NIL;
            }
        }
    }
