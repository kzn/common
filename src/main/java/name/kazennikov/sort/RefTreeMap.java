...
public class RefTreeMap<K, V> extends AbstractMap<K, V> implements
	 ...
	 */
	public RefTreeMap(Comparator<? super K> comparator) {
	 ...
	 */
	public RefTreeMap(Map<? extends K, ? extends V> m) {
		comparator = null;
		putAll(m);
	}
	 ...
	 *             if the specified map is null
	 */
	public RefTreeMap(SortedMap<K, ? extends V> m) {
	 ...
	 */
	public Object clone() {
		RefTreeMap<K, V> clone = null;
		try {
			clone = (RefTreeMap<K, V>) super.clone();
	...
	public Collection<V> values() {
		Collection<V> vs = null;// values;
		return (vs != null) ? vs : (null);// values = new Values());
	}
	...
	public NavigableMap<K, V> subMap(K fromKey, boolean fromInclusive, K toKey,
			boolean toInclusive) {
		return new AscendingSubMap(this, false, fromKey, fromInclusive, false,
				toKey, toInclusive);
	}
	...
	public NavigableMap<K, V> headMap(K toKey, boolean inclusive) {
		return new AscendingSubMap(this, true, null, true, false, toKey,
				inclusive);
	}
	...
	public NavigableMap<K, V> tailMap(K fromKey, boolean inclusive) {
		return new AscendingSubMap(this, false, fromKey, inclusive, true, null,
				true);
	}
	...
	class Values extends AbstractCollection<V> {
		...
		public int size() {
			return RefTreeMap.this.size();
		}

		public boolean contains(Object o) {
			return RefTreeMap.this.containsValue(o);
		}
	...
	static final class KeySet<E> extends AbstractSet<E> implements
		...
		public Iterator<E> iterator() {
			if (m instanceof RefTreeMap)
				return ((RefTreeMap<E, Object>) m).keyIterator();
			else
				return (Iterator<E>) (((RefTreeMap.NavigableSubMap) m)
						.keyIterator());
		}

		public Iterator<E> descendingIterator() {
			if (m instanceof RefTreeMap)
				return ((RefTreeMap<E, Object>) m).descendingKeyIterator();
			else
				return (Iterator<E>) (((RefTreeMap.NavigableSubMap) m)
		...
		public NavigableSet<E> subSet(E fromElement, boolean fromInclusive,
			...
			return new KeySet<>(m.subMap(fromElement, fromInclusive, toElement,
					toInclusive));
		}
	 ...
	 */
	static <K, V> Map.Entry<K, V> exportEntry(RefTreeMap.Entry<K, V> e) {
	 ...
	 */
	static <K, V> K keyOrNull(RefTreeMap.Entry<K, V> e) {
	...
	abstract static class NavigableSubMap<K, V> extends AbstractMap<K, V>
		...
		NavigableSubMap(RefTreeMap<K, V> m, boolean fromStart, K lo,
		...
		final RefTreeMap.Entry<K, V> absLowest() {
			RefTreeMap.Entry<K, V> e = (fromStart ? m.getFirstEntry()
					: (loInclusive ? m.getCeilingEntry(lo) : m
							.getHigherEntry(lo)));
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final RefTreeMap.Entry<K, V> absHighest() {
			RefTreeMap.Entry<K, V> e = (toEnd ? m.getLastEntry()
					: (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
			return (e == null || tooLow(e.key)) ? null : e;
		}

		final RefTreeMap.Entry<K, V> absCeiling(K key) {
			if (tooLow(key))
				return absLowest();
			RefTreeMap.Entry<K, V> e = m.getCeilingEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final RefTreeMap.Entry<K, V> absHigher(K key) {
			if (tooLow(key))
				return absLowest();
			RefTreeMap.Entry<K, V> e = m.getHigherEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final RefTreeMap.Entry<K, V> absFloor(K key) {
			if (tooHigh(key))
				return absHighest();
			RefTreeMap.Entry<K, V> e = m.getFloorEntry(key);
			return (e == null || tooLow(e.key)) ? null : e;
		}

		final RefTreeMap.Entry<K, V> absLower(K key) {
			if (tooHigh(key))
				return absHighest();
			RefTreeMap.Entry<K, V> e = m.getLowerEntry(key);
			return (e == null || tooLow(e.key)) ? null : e;
		}

		/** Returns the absolute high fence for ascending traversal */
		final RefTreeMap.Entry<K, V> absHighFence() {
			return (toEnd ? null : (hiInclusive ? m.getHigherEntry(hi) : m
					.getCeilingEntry(hi)));
		}

		/** Return the absolute low fence for descending traversal */
		final RefTreeMap.Entry<K, V> absLowFence() {
			return (fromStart ? null : (loInclusive ? m.getLowerEntry(lo) : m
					.getFloorEntry(lo)));
		}
		...
		abstract RefTreeMap.Entry<K, V> subLowest();

		abstract RefTreeMap.Entry<K, V> subHighest();

		abstract RefTreeMap.Entry<K, V> subCeiling(K key);

		abstract RefTreeMap.Entry<K, V> subHigher(K key);

		abstract RefTreeMap.Entry<K, V> subFloor(K key);

		abstract RefTreeMap.Entry<K, V> subLower(K key);
		...
		public final Map.Entry<K, V> pollFirstEntry() {
			RefTreeMap.Entry<K, V> e = subLowest();
		...
		public final Map.Entry<K, V> pollLastEntry() {
			RefTreeMap.Entry<K, V> e = subHighest();
			Map.Entry<K, V> result = exportEntry(e);
			if (e != null)
				m.deleteEntry(e);
			return result;
		...
		public final NavigableSet<K> navigableKeySet() {
			...
			return (nksv != null) ? nksv
					: (navigableKeySetView = new RefTreeMap.KeySet(this));
		}
		...
		abstract class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
			...
			public boolean isEmpty() {
				RefTreeMap.Entry<K, V> n = absLowest();
			...
			public boolean contains(Object o) {
				...
				if (!inRange(key))
					return false;
				RefTreeMap.Entry node = m.getEntry(key);
				return node != null
			...
			public boolean remove(Object o) {
				...
				if (!inRange(key))
					return false;
				RefTreeMap.Entry<K, V> node = m.getEntry(key);
				if (node != null
						&& valEquals(node.getValue(), entry.getValue())) {
					m.deleteEntry(node);
					return true;
		...
		abstract class SubMapIterator<T> implements Iterator<T> {
			RefTreeMap.Entry<K, V> lastReturned;
			RefTreeMap.Entry<K, V> next;
			final Object fenceKey;
			int expectedModCount;

			SubMapIterator(RefTreeMap.Entry<K, V> first, RefTreeMap.Entry<K, V> fence) {
			...
			final RefTreeMap.Entry<K, V> nextEntry() {
				RefTreeMap.Entry<K, V> e = next;
			...
			final RefTreeMap.Entry<K, V> prevEntry() {
				RefTreeMap.Entry<K, V> e = next;
				if (e == null || e.key == fenceKey)
					throw new NoSuchElementException();
				if (m.modCount != expectedModCount)
		...
		final class SubMapEntryIterator extends SubMapIterator<Map.Entry<K, V>> {
			SubMapEntryIterator(RefTreeMap.Entry<K, V> first,
					RefTreeMap.Entry<K, V> fence) {
				super(first, fence);
			}
		...
		final class SubMapKeyIterator extends SubMapIterator<K> {
			SubMapKeyIterator(RefTreeMap.Entry<K, V> first,
					RefTreeMap.Entry<K, V> fence) {
				super(first, fence);
			}
		...
		final class DescendingSubMapEntryIterator extends
				SubMapIterator<Map.Entry<K, V>> {
			DescendingSubMapEntryIterator(RefTreeMap.Entry<K, V> last,
					RefTreeMap.Entry<K, V> fence) {
				super(last, fence);
			}
		...
		final class DescendingSubMapKeyIterator extends SubMapIterator<K> {
			DescendingSubMapKeyIterator(RefTreeMap.Entry<K, V> last,
					RefTreeMap.Entry<K, V> fence) {
				super(last, fence);
			}
	...
	static final class AscendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		AscendingSubMap(RefTreeMap<K, V> m, boolean fromStart, K lo,
				boolean loInclusive, boolean toEnd, K hi, boolean hiInclusive) {
		...
		public NavigableMap<K, V> subMap(K fromKey, boolean fromInclusive,
			...
			return new AscendingSubMap(m, false, fromKey, fromInclusive, false,
					toKey, toInclusive);
		}
		...
		public NavigableMap<K, V> headMap(K toKey, boolean inclusive) {
			if (!inRange(toKey, inclusive))
				throw new IllegalArgumentException("toKey out of range");
			return new AscendingSubMap(m, fromStart, lo, loInclusive, false,
					toKey, inclusive);
		}
		...
		public NavigableMap<K, V> tailMap(K fromKey, boolean inclusive) {
			if (!inRange(fromKey, inclusive))
				throw new IllegalArgumentException("fromKey out of range");
			return new AscendingSubMap(m, false, fromKey, inclusive, toEnd, hi,
					hiInclusive);
		}
		...
		public NavigableMap<K, V> descendingMap() {
			...
			return (mv != null) ? mv
					: (descendingMapView = new DescendingSubMap(m, fromStart,
		...
		RefTreeMap.Entry<K, V> subLowest() {
			return absLowest();
		}

		RefTreeMap.Entry<K, V> subHighest() {
			return absHighest();
		}

		RefTreeMap.Entry<K, V> subCeiling(K key) {
			return absCeiling(key);
		}

		RefTreeMap.Entry<K, V> subHigher(K key) {
			return absHigher(key);
		}

		RefTreeMap.Entry<K, V> subFloor(K key) {
			return absFloor(key);
		}

		RefTreeMap.Entry<K, V> subLower(K key) {
			return absLower(key);
		}
	}
	...
	static final class DescendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		DescendingSubMap(RefTreeMap<K, V> m, boolean fromStart, K lo,
				boolean loInclusive, boolean toEnd, K hi, boolean hiInclusive) {
			super(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
		}
		...
		public NavigableMap<K, V> subMap(K fromKey, boolean fromInclusive,
			...
			if (!inRange(toKey, toInclusive))
				throw new IllegalArgumentException("toKey out of range");
			return new DescendingSubMap(m, false, toKey, toInclusive, false,
		...
		public NavigableMap<K, V> headMap(K toKey, boolean inclusive) {
			if (!inRange(toKey, inclusive))
				throw new IllegalArgumentException("toKey out of range");
			return new DescendingSubMap(m, false, toKey, inclusive, toEnd, hi,
					hiInclusive);
		}
		...
		public NavigableMap<K, V> tailMap(K fromKey, boolean inclusive) {
			if (!inRange(fromKey, inclusive))
				throw new IllegalArgumentException("fromKey out of range");
			return new DescendingSubMap(m, fromStart, lo, loInclusive, false,
					fromKey, inclusive);
		}
		...
		public NavigableMap<K, V> descendingMap() {
			NavigableMap<K, V> mv = descendingMapView;
			return (mv != null) ? mv
					: (descendingMapView = new AscendingSubMap(m, fromStart,
							lo, loInclusive, toEnd, hi, hiInclusive));
		}
		...
		final class DescendingEntrySetView extends EntrySetView {
			public Iterator<Map.Entry<K, V>> iterator() {
				return new DescendingSubMapEntryIterator(absHighest(),
						absLowFence());
			}
		}
	...
	private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>,
		...
		private Object readResolve() {
			return new AscendingSubMap(RefTreeMap.this, fromStart, fromKey, true,
					toEnd, toKey, false);
		}
	 ...
	 */
	static <K, V> RefTreeMap.Entry<K, V> successor(Entry<K, V> t) {
		if (t == null)
			return null;
