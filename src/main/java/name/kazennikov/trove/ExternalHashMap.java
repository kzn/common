...
public class ExternalHashMap<K, V> extends AbstractMap<K, V> implements
	 ...
	 */
	public ExternalHashMap(int initialCapacity) {
	 ...
	 */
	public ExternalHashMap(Map<? extends K, ? extends V> m) {
	 ...
	 */
	public Object clone() {
		ExternalHashMap<K, V> result = null;
		try {
			result = (ExternalHashMap<K, V>) super.clone();
		...
		if (result.table != EMPTY_TABLE) {
			result.inflateTable(Math.min(
					...
					// we have limits...
							ExternalHashMap.MAXIMUM_CAPACITY), table.length));
		}
	...
	static class Entry<K, V> implements Map.Entry<K, V> {
		 ...
		 * HashMap.
		 */
		void recordAccess(ExternalHashMap<K, V> m) {
		}

		/**
		 * This method is invoked whenever the entry is removed from the table.
		 */
		void recordRemoval(ExternalHashMap<K, V> m) {
		}
	}
	...
	private abstract class HashIterator<E> implements Iterator<E> {
		...
		public void remove() {
			...
			current = null;
			ExternalHashMap.this.removeEntryForKey(k);
			expectedModCount = modCount;
		}
	}
	...
	private final class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return ExternalHashMap.this.removeEntryForKey(o) != null;
		}

		public void clear() {
			ExternalHashMap.this.clear();
		}
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws IOException,
		...
		int capacity = (int) Math.min(
				...
				// we have limits...
				ExternalHashMap.MAXIMUM_CAPACITY);
