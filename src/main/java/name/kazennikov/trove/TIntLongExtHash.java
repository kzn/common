...
abstract public class TIntLongExtHash extends TPrimitiveHash {
	 ...
	 *
	 * NOTE: should not be modified after the Hash is created, but is not final
	 * because of Externalization
	 *
	 */
	...
	/**
	 * Creates a new <code>T#E#Hash</code> instance with the default capacity
	 * and load factor.
	 */
	public TIntLongExtHash() {
		super();
		no_entry_key = (int) 0;
	...
	/**
	 * Creates a new <code>T#E#Hash</code> instance whose capacity is the next
	 ...
	 */
	public TIntLongExtHash(TIntHashingStrategy hasher, int initialCapacity) {
	...
	/**
	 * Creates a new <code>TIntIntHash</code> instance with a prime value at or
	 ...
	 *            place.
	 */
	public TIntLongExtHash(TIntHashingStrategy hasher, int initialCapacity, float loadFactor) {
	 ...
	 * value is generally zero, but can be changed during construction of the
	 * collection.
	 *
	...
	protected int index(int key) {
		...
		hash = hasher.computeHashCode(key) & 0x7fffffff;
		index = hash % length;
		...
		if (state == FULL && hasher.equals(set[index], key))
			return index;
	...
	int indexRehashed(int key, int index, int hash, byte state) {
		...
		do {
			...
			//
			if (hasher.equals(key, _set[index]) && state != REMOVED)
				return index;
	...
	protected int insertKey(int val) {
		...
		hash = hasher.computeHashCode(val) & 0x7fffffff;
		...
		if (state == FULL && hasher.equals(_set[index], val)) {
	...
	protected int XinsertKey(int key) {
		...
		if (state == FREE) {
			...
			return index; // empty, all done
		} else if (state == FULL && hasher.equals(set[index], key)) {
			return -index - 1; // already stored
		...
		// compute the double hash
			...
			if (state != REMOVED) {
				...
				do {
					...
					state = states[index];
				} while (state == FULL && !hasher.equals(set[index],key));
			}
			...
			if (state == REMOVED) {
				...
				while (state != FREE && (state == REMOVED || hasher.equals(set[index], key))) {
					index -= probe;
					if (index < 0) {
						index += length;
					}
